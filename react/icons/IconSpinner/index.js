...
class IconSpinner extends PureComponent {
  render() {
    const { className, animate, pulse, ignoreSize, width, height, color } = this.props
    const style = !ignoreSize ? { width: width, height: height } : {}
    const svgColor = color === 'currentColor' || !colors[color] ? 'currentColor' : colors[color]

    return (
      <svg
        className={className + ' ' + (animate || pulse ? styles.iconAnimate : '')}
        {...style}
...
IconSpinner.propTypes = {
  ...
  pulse: PropTypes.bool // the same of "animate". just for not depreciate "pulse"
}
